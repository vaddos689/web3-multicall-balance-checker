...
from core.modules.utils.contracts.contract import MULTICALL_ETH_CONTRACTS, MULTICALL_CONTRACTS
from config import ERC20_ABI, MULTICALL_ABI
from datas.data import DATA
from core.modules.utils.helpers import decimalToInt
import math
...
class Multicall():
        ...
        try:
            ...
            aggregate3_abi = {"inputs": [{"components": [
                ...
                {"internalType": "bytes", "name": "callData", "type": "bytes"}],
                                          "internalType": "struct Multicall3.Call3[]", "name": "calls",
                                          "type": "tuple[]"}], "name": "aggregate3", "outputs": [{"components": [
    ...
    async def get_token_symbols(self, token_list):
        ...
        try:
            ...
            decimals_list = [self._normalize_output(output_types, byte_values) for (success, byte_values) in
                             multicall_result]
    ...
    async def get_token_decimals(self, token_list):
        ...
        len_batch = 1000
        all_calls = [list_calls[i:i + len_batch] for i in
                     range(0, len(list_calls), len_batch)]  # делим запросы на части по 1000
    ...
    async def get_eth_balance_zksync_lite_batch(self, wallets, convert_from_wei=True):
        ...
        async with aiohttp.ClientSession() as session:
            for i, wallets_batch in enumerate(wallets_list):
                tasks = [self.fetch_eth_balance(session, user_address, convert_from_wei) for user_address in
                         wallets_batch]
        ...
        try:
            ...
            for lens, wallets_list in enumerate(wallets_batches, start=1):
                ...
                multicall_contract = self.web3.eth.contract(
                    address=Web3.to_checksum_address(MULTICALL_ETH_CONTRACTS[self.chain]), abi=abi)
    ...
    async def get_erc20_balances(self, wallets, token_list, symbols_list, decimals_list, convert_from_wei=True):
        ...
        for i, user_address in enumerate(wallets, start=1):
            ...
            for token_address in token_list:
                ...
                list_calls.append({
                    'target': Web3.to_checksum_address(token_address),
                    'allowFailure': False,
                    'callData': token_contract.encodeABI(fn_name='balanceOf',
                                                         args=[Web3.to_checksum_address(user_address)])
                })
        print()
        ...
        for lens, calls in enumerate(all_calls, start=1):
            multicall_result = await self.aggregate3(calls)
            balance_list = [self._normalize_output(output_types, byte_values) for (success, byte_values) in
                            multicall_result]
    ...
    async def get_allowances(self, wallets, spender_list, token_list, convert_from_wei=True):
        ...
        try:
            ...
            multicall_result = await self.aggregate3(calls)
            allowance_list = [self._normalize_output(output_types, byte_values) for (success, byte_values) in
                              multicall_result]
